...
- JSON 상태 파일 쓰기는 파일별 `.lock` lease를 잡고 원자 교체한다. 기존 정상 파일은 `.bak`로 남기며, 대표 상태 저장소는 손상된 JSON을 읽으면 백업을 검증해 복구를 시도한다. `doctor --json`의 workspace 체크는 상태 JSON, 백업, lock, 손상 JSON 개수를 노출한다.
