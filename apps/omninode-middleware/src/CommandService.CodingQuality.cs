...
public sealed partial class CommandService
{
    ...
    private static bool HasCodingQualityFailure(CodingWorkerResult worker)
    {
    ...
    private static int ScoreCodingWorkerResult(CodingWorkerResult worker)
    {
        var score = 0;
        var status = (worker.Execution.Status ?? string.Empty).Trim();
        ...
        if (HasCodingQualityFailure(worker))
        {
            score -= 80;
        }
    ...
    private static CodingWorkerResult SelectBestCodingWorkerResult(IReadOnlyList<CodingWorkerResult> workers, CodingWorkerResult fallback)
    {
    ...
    private static IReadOnlyList<string> MergeChangedFilesForBestWorker(
