...
- `dotnet test apps/omninode-middleware-tests/OmniNode.Middleware.Tests.csproj`: 통과, 841 tests
- `node scripts/check-security-boundaries.mjs`: 통과, assertions 721
...
- `apps/omninode-middleware/src/LogicValueParsingPolicy.cs`
  - 로직 그래프에서 쓰는 순수 값 파서/평가기(`FirstNonEmpty`, `ParseCsvValues`, `ParseMultilineValues`, `ParsePositiveInt`, `ParseOptionalInt`, `ParseDouble`, `ParseBool`, `ParseOptionalBool`, `CompareNumbers`, `EvaluateCondition`)를 `CommandService.LogicGraphs`에서 분리했다.
  - `EvaluateCondition`은 `LogicGraphValidationPolicy.NormalizeOperator`를 사용해 alias(`=`, `==`, `!=`, `<>`, `>=`, `<=`, `startswith`, …)를 동일하게 처리한다.
  - `CommandService.LogicGraphs.cs`의 `FirstNonEmpty`/`ParseCsvValues`/`ParseMultilineValues`/`ParseLogicPositiveInt`/`ParseOptionalLogicInt`/`ParseLogicDouble`/`ParseLogicBool`/`ParseOptionalLogicBool`/`CompareLogicNumbers`/`EvaluateLogicCondition` 사설 메서드를 제거했다.
  - `CommandService.LogicGraphs.cs` 본문 크기: 2820 → 2689 라인.
- `apps/omninode-middleware-tests/LogicValueParsingPolicyTests.cs`
  - 빈값/공백 fallback, csv/multiline split의 trim·중복 제거·case-insensitive dedup, 정수 clamp, optional int blank 처리, double fallback, truthy 토큰 매칭(1/true/yes/y/on), optional bool blank 처리, 숫자 비교의 fallback-zero, condition 평가의 11개 normalize 연산자, alias 매칭(`==`/`!=`/`>=`/`startswith`/unknown→equals fallback)을 단위 테스트로 고정했다 (46 cases).
- `scripts/check-security-boundaries.mjs`
  - `LogicValueParsingPolicy` 소유권과 `CommandService.LogicGraphs` 위임/제거 계약을 추가했다 (assertions 529 → 551).
- `apps/omninode-middleware/src/LogicNodeRuntimePolicy.cs`
  - 로직 그래프 노드 실행 흐름의 순수 판정을 단일 정책으로 모았다: `TemplateRegex` (`{{ expr }}` 컴파일 regex), `IsSuccessfulExecutionStatus`, `IsTerminalStatus`, `LooksLikeAiFailure`, `IsRunInputTemplate`, `ShouldApplyImplicitMainInput`, `RequiresAllIncomingEdges`, `IsNodeReadyToRun`, `BuildConversationTitle`.
  - AI 실패 신호(에러 접두사, API 키 누락, 인증 필요, 호출 오류, 요청 실패, 응답 시간 초과)와 종결 상태(`completed`/`error`/`failed`/`canceled`/`cancelled`/`timeout`/`killed`)를 정책 클래스가 소유한다.
  - `RequiresAllIncomingEdges`/`IsNodeReadyToRun`은 `LogicGraphValidationPolicy.NormalizePort`를 사용해 main vs 분기 포트 판정을 재사용한다.
  - `CommandService.LogicGraphs.cs`에서 사설 `LogicTemplateRegex` static과 `IsSuccessfulLogicExecutionStatus`/`LooksLikeLogicAiFailure`/`IsLogicRunInputTemplate`/`ShouldApplyImplicitMainInput`/`RequiresAllLogicIncomingEdges`/`IsLogicNodeReadyToRun`/`BuildLogicConversationTitle`/`IsTerminalLogicStatus` 사설 메서드와 더 이상 필요 없어진 `using System.Text.RegularExpressions`를 모두 제거했다.
  - `CommandService.LogicGraphs.cs` 본문 크기: 2689 → 2562 라인 (분리 누적 3110 → 2562, ≈ 17.6% 감축).
- `apps/omninode-middleware-tests/LogicNodeRuntimePolicyTests.cs`
  - 성공/종결 상태 토큰 매칭, AI 실패 휴리스틱의 빈값/error 접두사/한국어 실패 문구, run-input 템플릿 판정(pure `{{run.input}}`만 허용, prefix/suffix/다른 변수 거부), implicit main input 정책의 reference/edge 모드 거부와 blank/template/literal 분기, require-all-incoming의 분기 포트 감지, parallel_join 노드 준비 상태(부분 도착 거부, 완전 도착 허용), 시작 노드 즉시 준비, 대화 제목 조립, `{{vars.name}}` 정규식 추출을 단위 테스트로 고정했다 (47 cases).
- `scripts/check-security-boundaries.mjs`
  - `LogicNodeRuntimePolicy` 소유권과 `CommandService.LogicGraphs`의 사설 helper/static regex 제거 계약을 추가했다 (assertions 551 → 574).
- `apps/omninode-middleware/src/CodingOrchestrationPromptPolicy.cs`
  - 오케스트레이션 코딩의 단계별 프롬프트(`BuildPlannerPrompt`, `BuildImplementationPrompt`, `BuildVerificationPrompt`, `BuildFixPrompt`), 다중 워커 프롬프트(`BuildMultiWorkerPrompt`), 자동 모드 프롬프트(`BuildAutoInput`)를 `CommandService.Coding`에서 분리했다.
  - 프롬프트는 `CodingWorkerResult.Summary`만 의존하므로 정책 클래스가 텍스트 조립 책임을 전담한다.
  - `CommandService.Coding.cs`에서 사설 `BuildOrchestrationPlannerPrompt`/`BuildOrchestrationImplementationPrompt`/`BuildOrchestrationVerificationPrompt`/`BuildOrchestrationFixPrompt`/`BuildMultiCodingWorkerPrompt`/`BuildAutoOrchestrationCodingInput` 6개 사설 메서드를 제거했다.
  - `CommandService.Coding.cs` 본문 크기: 1954 → 1813 라인.
- `apps/omninode-middleware-tests/CodingOrchestrationPromptPolicyTests.cs`
  - planner/implementation/verification/fix 프롬프트의 단계 표시, 언어 힌트, 역할 텍스트, 이전 단계 요약 forwarding, 더미 금지 규칙, multi worker 프롬프트의 협의 금지 규칙, auto input 프롬프트의 blank 언어 fallback(`auto`)을 단위 테스트로 고정했다 (6 cases).
- `scripts/check-security-boundaries.mjs`
  - `CodingOrchestrationPromptPolicy` 소유권과 `CommandService.Coding` 위임/제거 계약을 추가했다 (assertions 574 → 592).
- `apps/omninode-middleware/src/TextOutputTruncator.cs`
  - 4개 위치(`CodingFallbackPolicy`, `CodingLoopTuningPolicy`, `CodingProgressPolicy`, `CommandService.Telegram`)에 중복 정의돼 있던 `TrimForOutput` 의미를 단일 유틸리티로 통합했다.
  - 3가지 의미를 보존: `TruncateWithMin200`(min-clamp 200 + `...(truncated)` marker, telegram/progress), `TruncateRaw`(min 없음 + 같은 marker, loop tuning), `TruncateWithEllipsis`(trim + `...` marker, fallback).
  - 각 사용처는 기존 사설 `TrimForOutput`을 단일 줄 위임으로 축소했고, marker constant `DefaultTruncationMarker`를 정책 클래스가 소유한다.
- `apps/omninode-middleware-tests/TextOutputTruncatorTests.cs`
...
- `apps/omninode-middleware/src/Infrastructure/Search/SearchAnswerFormatterPolicy.cs`
  - `EnsureReadableWebAnswerResponse(text, input, allowMarkdownTable)`를 추가해 빈 입력 fallback, table 모드 분기, 출처 링크 잔재 제거, bullet 줄 정규화, 비교 요청 raw 보존, 목록 요청 시 numbered list 정규화, 일반 응답의 narrative paragraph 정규화를 한 곳에서 처리한다.
  - 분기에서 `SearchQueryPolicy.LooksLikeTableRenderRequest`/`LooksLikeComparisonRequest`/`LooksLikeListOutputRequest`를 직접 호출해 CommandService 내부 wrapper 의존을 끊었다.
- `apps/omninode-middleware/src/CommandService.SearchPipeline.cs`
  - 단순 위임만 하던 사설 wrapper 9개(`EnsureReadableWebAnswerResponse`/`NormalizeGeminiWebNumberedListResponse`/`LooksLikeNumberedListResponse`/`NormalizeCollapsedWebBulletRuns`/`RemoveWebAnswerSourceLinkArtifacts`/`NormalizeWebAnswerNarrativeParagraphs`/`EnsureMarkdownTableResponseIfRequested`/`ConvertDelimitedPlainTextTableToMarkdown`/`NormalizeMarkdownTableResponseMetadata`/`SanitizeTableCell`)를 제거하고 호출부를 `SearchAnswerFormatterPolicy.X` 직접 호출로 정리했다.
  - `CommandService.SearchPipeline.cs` 본문 크기: 1070 → 994 라인.
- `apps/omninode-middleware/src/CommandService.Citations.cs`
  - 표 셀 sanitization 호출을 `SearchAnswerFormatterPolicy.SanitizeTableCell` 직접 호출로 통일했다.
- `apps/omninode-middleware-tests/SearchAnswerFormatterPolicyTests.cs`
  - `EnsureReadableWebAnswerResponse`의 blank/numbered list/comparison passthrough/table mode 비허용 분기 4건을 추가 단위 테스트로 고정했다 (기존 5 + 신규 4 = 9 cases).
- `scripts/check-security-boundaries.mjs`
  - `SearchAnswerFormatterPolicy.EnsureReadableWebAnswerResponse` 소유권과 `CommandService.SearchPipeline`/`CommandService.Citations`의 위임/제거 계약을 추가했다 (assertions 625 → 632).
- `apps/omninode-middleware/src/ChatRetryGuardPolicy.cs`
  - 단일 챗의 retry/guard 휴리스틱 8종(`ShouldRetryWithoutHistory`, `LooksLikeOffTopicModelExplanation`, `LooksLikeUnrequestedP2SAnswer`, `BuildOffTopicGuardMessage`, `LooksLikeVagueWebLookupRequest`, `ResolveSingleChatMaxOutputTokens`, `BuildHistoryBypassInput`, `BuildOriginalRequestRetryInput`)을 단일 정책으로 모았다.
  - 뉴스 요청 skip, gpt-oss off-topic 감지, P2S 미요청 답변 감지, vague lookup 짧은 문구 매칭, 단일 챗 max-output-token 버킷(4096/3072/2048)과 history-bypass/original-retry 프롬프트 wrapping을 정책 클래스가 소유한다.
  - `ResolveSingleChatMaxOutputTokens`는 `SearchQueryPolicy.LooksLikeListOutputRequest`를 직접 호출해 CommandService 내부 wrapper 의존을 끊었다.
  - `CommandService.Chat`의 `ShouldRetrySingleChatWithoutHistory`/`LooksLikeOffTopicModelExplanation`/`LooksLikeUnrequestedP2SAnswer`/`BuildOffTopicGuardMessage`/`LooksLikeVagueWebLookupRequest`/`ResolveSingleChatMaxOutputTokens`/`BuildHistoryBypassInput`/`BuildOriginalRequestRetryInput` 8개 사설 메서드를 제거했고, `CommandService.Telegram`의 6개 호출처도 정책 호출로 갱신했다.
  - `CommandService.Chat.cs` 본문 크기: 2702 → 2502 라인.
- `apps/omninode-middleware-tests/ChatRetryGuardPolicyTests.cs`
  - blank 입력 fallback, 뉴스 요청 skip, gpt-oss off-topic 감지, P2S 미요청 답변 감지, off-topic 가드 메시지 분기, vague lookup 길이/문구 매칭, max-output-token 버킷 매핑, history-bypass/original-retry 프롬프트 wrapping과 blank fallback을 단위 테스트로 고정했다 (23 cases).
- `scripts/check-security-boundaries.mjs`
  - `ChatRetryGuardPolicy` 소유권과 `CommandService.Chat`의 사설 helper 8개 제거, `CommandService.Telegram`의 위임 계약을 추가했다 (assertions 632 → 651).
- `apps/omninode-middleware/src/LocalAssistantQuestionPolicy.cs`
  - 어시스턴트 자기 정보 질의 4종(`LooksLikeSkillInventoryQuestion`, `LooksLikeLimitationQuestion`, `LooksLikeCapabilityQuestion`, `LooksLikeIdentityQuestion`), 스킬 이름 경계 매칭(`IndexOfSkillNameWithBoundary`, `IsSkillNameBoundaryInside`), 로컬 정보 텍스트 트리밍(`TrimAssistantInfoText`, `TrimToUtf8ByteCount`)을 단일 정책으로 모았다.
  - 한국어/영어 의문 표지어 패턴, 조사 접미사, 압축 토큰 매칭, "너/넌/네가/당신/ai/omni" 등 어시스턴트 대상 필터, ASCII 영숫자+`_`/`-` 기준 단어 경계 분류를 정책 클래스가 소유한다.
  - `CommandService.Chat`의 `LooksLikeLocalSkillInventoryQuestion`/`LooksLikeLocalLimitationQuestion`/`LooksLikeLocalCapabilityQuestion`/`LooksLikeLocalIdentityQuestion`/`IndexOfSkillNameWithBoundary`/`IsSkillNameBoundaryInside`/`TrimLocalAssistantInfoText`/`TrimToUtf8ByteCount` 8개 사설 메서드와 미사용 `FirstNonEmptyLocal` helper를 제거했고, `CommandService.Telegram.Skills`의 3개 호출처도 정책 호출로 갱신했다.
  - `CommandService.Chat.cs` 본문 크기: 2502 → 2261 라인.
- `apps/omninode-middleware-tests/LocalAssistantQuestionPolicyTests.cs`
  - 스킬 인벤토리 의문 패턴/조사/압축 매칭과 false-positive(`eli5 스킬을 사용해서…`) 차단, limitation/capability 질문의 어시스턴트 대상 필터, identity 질문의 한국어/영어 cue 매칭, 단어 경계 기반 스킬 이름 탐색의 false-positive 차단(`aim` vs `ai`), UTF-8 byte trim의 rune 경계 처리, blank fallback을 단위 테스트로 고정했다 (34 cases).
- `scripts/check-security-boundaries.mjs`
  - `LocalAssistantQuestionPolicy` 소유권과 `CommandService.Chat`의 사설 helper 8개 + `FirstNonEmptyLocal` 미사용 helper 제거, `CommandService.Telegram.Skills`의 위임 계약을 추가했다 (assertions 651 → 672).
- `apps/omninode-middleware/src/RoutineSchedulePolicy.cs`
  - 루틴 스케줄 정규화/포맷/파싱 helper 19종(`ResolveTimeZone`, `NormalizeScheduleKind`, `NormalizeWeekdays`, `NormalizeRetryCount`, `NormalizeRetryDelaySeconds`, `NormalizeNotifyPolicy`, `IsRetryableStatus`, `ComputeOutputFingerprint`, `FormatWeekday`, `FormatWeekdays`, `BuildScheduleDisplay`, `BuildDailyConfig`, `TryParseTimeOfDay`, `ParseWeekdayToken`, `TryExtractNaturalTime`, `TryExtractMonthlyDay`, `TryExtractWeekdays`, `ParseDailySchedule`, `TryBuildConfig`)을 단일 정책으로 모았다.
  - 타임존 fallback(Local), HH:MM/한국어 자연어 시각 파싱(`아침/오전/오후/저녁/밤/새벽` 12시 보정, `반` 30분), 평일/주말/요일 매칭, retry/notify 정책 normalization, daily/weekly/monthly RoutineScheduleConfig 조립, SHA-256 출력 fingerprint를 정책 클래스가 소유한다.
  - `CommandService.RoutineManagement` 18개 사설 메서드(`ResolveTimeZone`/`NormalizeRoutineScheduleKind`/`NormalizeRoutineWeekdays`/`NormalizeRoutineRetryCount`/`NormalizeRoutineRetryDelaySeconds`/`NormalizeRoutineNotifyPolicy`/`IsRoutineRetryableStatus`/`ComputeRoutineOutputFingerprint`/`FormatRoutineWeekdays`/`FormatRoutineWeekday`/`BuildRoutineScheduleDisplay`/`BuildDailyRoutineScheduleConfig`/`TryParseRoutineTimeOfDay`/`ParseRoutineWeekdayToken`/`TryExtractRoutineNaturalTime`/`TryExtractRoutineMonthlyDay`/`TryExtractRoutineWeekdays`/`ParseDailySchedule`/`TryBuildRoutineScheduleConfig`)를 제거했고, 6개 파일(Config/LogicGraphs/Routines/RoutineExecution/RoutineManagement/SearchEvidencePackBuilder)의 호출처를 정책 호출로 갱신했다.
  - `SearchEvidencePackBuilder`의 자체 `ResolveTimeZone`은 UTC fallback이라 정책과 의미가 달라 로컬 helper로 유지했다.
  - `CommandService.RoutineManagement.cs` 본문 크기: 2207 → 1772 라인 (≈ 19.7% 감축).
- `apps/omninode-middleware-tests/RoutineSchedulePolicyTests.cs`
...
| P3. CommandService 도메인 분리 | 진행 중 | 99.99% | logic graph 인스턴스-서비스 의존 노드 실행기(file/web/chat/coding/session/cron/browser/routine), exception/loop recovery orchestration — 모두 instance state/service 강결합으로 추출 시 DI 플럼빙 비용이 이득보다 큼 |
...
### 추가 진행 (로직 그래프 순수 노드 실행기 분리, 2026-05-21)

- `apps/omninode-middleware/src/Application/Logic/LogicGraphModels.cs`
  - `CommandService` 내부 private nested `LogicNodeExecutionOutcome` 레코드를 top-level `internal sealed record`로 승격했다 (envelope + if 분기 라벨).
- `apps/omninode-middleware/src/LogicLeafNodeExecutor.cs`
  - 인스턴스 서비스에 의존하지 않는 순수 노드 실행기 7종(start/end/output/template/set_var/if/parallel pass)을 분리했다. config/context와 `LogicTemplateResolver`/`LogicValueParsingPolicy`/`LogicGraphValidationPolicy`만 사용한다.
  - `ResolveLogicCondition`을 `ResolveCondition`으로 옮기고, envelope 조립 helper와 `MainInputKey`(`"__main_input"`) 단일 소스를 정책이 소유한다.
- `apps/omninode-middleware/src/CommandService.LogicGraphs.cs`
  - 사설 `ExecuteLogicStartNode`/`ExecuteLogicEndNode`/`ExecuteLogicOutputNode`/`ExecuteLogicTemplateNode`/`ExecuteLogicSetVarNode`/`ExecuteLogicIfNode`/`ExecuteLogicPassNode` 7개 메서드와 `ResolveLogicCondition`을 제거하고 dispatch switch를 `LogicLeafNodeExecutor.*` 호출로 갱신했다.
  - `LogicResolvedMainInputKey` const를 `LogicLeafNodeExecutor.MainInputKey` 참조로 바꿔 단일 소스를 유지했다.
  - file/web/chat/coding/session/cron/browser/routine 노드 실행기는 인스턴스 서비스(`_memorySearchTool`, `_conversationStore`, LLM 라우터, 파일 IO 등) 의존이 강해 추출하지 않았다.
  - `CommandService.LogicGraphs.cs` 본문 크기: 2421 → 2215 라인.
- `apps/omninode-middleware-tests/LogicLeafNodeExecutorTests.cs`
  - start의 run-input/fallback, end의 result→text→main-input→last-node→기본 폴백 우선순위, output 빈 폴백, template render, set_var 저장/이름 누락 거부, if의 true/false 분기, parallel pass의 main-input/fallback, ResolveCondition의 node config 폴백·operator 기본값을 단위 테스트로 고정했다 (8 cases).
- `scripts/check-security-boundaries.mjs`
  - `LogicLeafNodeExecutor` 소유권과 `CommandService.LogicGraphs`의 노드 실행기 위임/제거, ResolveLogicCondition 제거 계약을 추가했다 (assertions 711 → 721).
