...
function main() {
  ...
  assertNotIncludes(utils, "이번 수정에서는 pygame, pyglet, arcade 같은 외부 패키지와 pip install 시도를 금지한다", "no repair dependency ban");
  assertIncludes(utils, "OMNI_HEADLESS_TEST=1 smoke 실행", "repair headless guidance");
  ...
  assertIncludes(utils, "\"\" or \"auto\" => \"auto\"", "auto language preserved");
  assertIncludes(utils, "ResolveCodingProjectProfile(objective, \"auto\", requestedPaths).PrefersMultiFile", "bundle fallback defaults to project profile");
  assertIncludes(utils, "일반 코딩 요청은 여러 파일 프로젝트 구조를 우선하라", "bundle prompt multi-file default");
  assertIncludes(utils, "projectProfile", "bundle schema includes project profile");
  assertIncludes(utils, "\"react\" or \"vite\" or \"react-vite\"", "react vite normalization");
  assertIncludes(utils, "normalizedLanguage is \"javascript\" or \"typescript\" or \"react-vite\"", "frontend deferred command guard");
  assertIncludes(utils, "\"golang\" => \"go\"", "go normalization");
  assertIncludes(utils, "\"rs\" or \"cargo\" => \"rust\"", "rust normalization");
  assertIncludes(utils, "go|rs|php|rb|swift", "extended path regex");
  assertIncludes(utils, "[quality-gate] 미충족 항목", "quality repair guidance");
  assertIncludes(utils, "테스트용 초기화 로그, 빈 함수, 껍데기 UI, placeholder 데이터만으로 완료 처리 금지", "common dummy ban");
  assertIncludes(utils, "package.json 의 dependencies/devDependencies", "node dependency repair guidance");
  assertIncludes(utils, "csproj PackageReference", "dotnet repair guidance");
  assertIncludes(utils, "build.gradle/pom 설정", "jvm repair guidance");
  assertIncludes(utils, "Makefile/CMakeLists.txt", "native repair guidance");
